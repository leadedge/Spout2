...
Frame count allows receivers to determine sender frame rate (fps) and frame number. Applications for Spout versions previous to 2.007 will not produce frame information and the setting will not have any effect.
...
Memory sharing mode is a backup where hardware does not support texture sharing. It is not recommended if texture sharing is available.

All Spout applications built with Spout 2.006 SDK or greater will switch to the selected mode. 

Spout 2.006 enabled "CPU share" mode for CPU access to shared textures. This has been removed for Spout 2.007 due to poor performance and no advantage.
