...
bool SpoutControls::FindControls(std::string &mapname)
{
	...
	if(spoututils::ReadPathFromRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlMap", path) ) {
		if(path[0] > 0) {
			mapname = path;
			return true;
		}
...
bool SpoutControls::OpenControls(std::string mapname)
{
	...
	if (spoututils::WritePathToRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlPath", Path)) {
	...
	// printf("ControlMap(%s)\n", Path);
	if (spoututils::WritePathToRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlMap", Path)) {
	...
	if(filecontrols.size() > 0) {
		...
		spoututils::WritePathToRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlFile", m_ControlFilePath);
...
bool SpoutControls::OpenSpoutController(std::string CommandLine)
{
	...
	if(spoututils::ReadPathFromRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\SpoutControls", "InstallPath", InstallPath)) {
...
bool SpoutControls::FindControlFile(std::string &filepath)
{
	...
	if(spoututils::ReadPathFromRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlFile", path) ) {
		if(path[0] > 0) {
...
bool SpoutControls::Cleanup()
{
	...
	if(m_hSlot) {
		...
		spoututils::RemovePathFromRegistry(HKEY_CURRENT_USER, "Software\\Leading Edge\\Spout", "ControlMap");
		CloseHandle(m_hSlot);
		m_hSlot = NULL;
	}
