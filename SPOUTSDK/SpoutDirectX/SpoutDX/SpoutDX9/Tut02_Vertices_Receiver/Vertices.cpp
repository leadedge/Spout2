...
spoutDX9 receiver;
LPDIRECT3DTEXTURE9 g_pReceivedTexture = nullptr; // Texture received from a sender
...
HRESULT InitD3D( HWND hWnd )
{
	...
	// EnableSpoutLogFile("Vertices DX9 receiver.log"); // Log to file
	...
	if (!receiver.OpenDirectX9(hWnd))
		return E_FAIL;

	// Use the SpoutDX9 class object and device
	g_pD3D = receiver.GetDX9object();
	g_pd3dDevice = receiver.GetDX9device();
...
INT WINAPI wWinMain( HINSTANCE hInst, HINSTANCE, LPWSTR, INT )
{
    ...
    HWND hWnd = CreateWindow( L"D3D Tutorial",
							L"D3D Tutorial 02: Vertices - Spout receiver",
