...
=========================================================================
	...
	This is an example using SpoutGL SendImage, SendTexture and SendFbo
	...
	bool SendFbo(GLuint FboID,
	                 unsigned int fbowidth, unsigned int fboheight,
					 bool bInvert)
	...
	This application can be built without dependencies.
	More options are available for other OpenGL appications.
	Openframeworks examples also demonstrate options such as
	synchronisation, data exchange and utility functions.
...
unsigned char *g_pixelBuffer = nullptr; // Pixel buffer (also used for SendImage)
GLuint g_TextureGL = 0;                 // For SendTexture
GLuint g_FboGL = 0;                     // For SendFbo
	...
	_In_ int       nCmdShow)
{
	...
	// See details in the Openframeworks Sender > SenderGraphics example 
	...
	if (g_TextureGL) 
		glDeleteBuffers(1, &g_TextureGL);

	if (g_FboGL) 
		glDeleteBuffers(1, &g_FboGL);
...
void Render()
{
	...
	// Option to send the default OpenGL framebuffer
	// is not applicable for this example.
	// See details in the Openframeworks Sender > SenderGraphics example
...
// Send fbo (the fbo must be bound)
void SendFbo() {
	// Get current fbo binding
	GLint HostFbo = 0;
	glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &HostFbo);
	// Bind the fbo to send
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, g_FboGL);
	// Attach the texture for sending
	glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, g_TextureGL, 0);
	// Send the fbo (completeness is checked by the function)
	sender.SendFbo(g_FboGL, g_SenderWidth, g_SenderHeight, false);
    ...
    case WM_PAINT:
        {
			...
			// This also updates the fbo because the texture is attached to it
