...
void Render()
{
	...
	// Because Windows bitmaps are bottom-up, the pixel buffer is flipped ready for
	// WM_PAINT but it could also be drawn upside down. WM_PAINT requires BGRA format.
			...
			case IDM_ABOUT:
				{
					...
					about += "         Spout image receiver example\n\n";
					...
					SpoutMessageBox(NULL, (LPSTR)about.c_str(), "WinSpoutGLreceiver", MB_USERICON | MB_OK);
				}
				break;
			...
			if (pixelBuffer) {
				...
				if (!receiver.IsConnected()) {
					...
					if (g_SenderFormat == 87 || g_SenderFormat == 88) {
						...
						//
						// Received data is RGBA but windows draw is BGRA
