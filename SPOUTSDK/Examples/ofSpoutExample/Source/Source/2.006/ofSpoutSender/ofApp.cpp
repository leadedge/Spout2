/*
	...
	Copyright (C) 2015-2021 Lynn Jarvis.
...
void ofApp::setup(){
	...
	strcpy_s(sendername, 256, "OF Spout Graphics Sender");	// Set the sender name
	...
	myTextureImage.load("SpoutBox1.png"); // Load a texture image for the demo
...
void ofApp::draw() {
	...
	ofRotateYDeg(rotX); // rotate - must be float
	ofRotateXDeg(rotY);
	myTextureImage.getTexture().bind(); // bind our texture
	ofDrawBox(0.4*(float)ofGetHeight()); // Draw the graphic
	myTextureImage.getTexture().unbind(); // bind our texture
	ofPopMatrix();
	rotX+=0.6;
	rotY+=0.6;
