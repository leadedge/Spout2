/*
    ...
    Include SpoutDLL.h and Spout2.lib in the "src" source files folder
    Include Spout2.lib in : Project > Build Options > Linker Settings > Link libraries
    Include Spout2.dll in the "bin" executable folder
...
void ofApp::setup(){
	...
	ofSetWindowTitle("CB Spout Receiver");
...
void ofApp::draw(){
	...
	if(!bInitialized) {
		...
		if(CreateReceiver(SenderName, width, height, true)) {
	...
	if(bInitialized) {
		...
		if(ReceiveTexture(SenderName, width, height, myTexture.getTextureData().textureID, myTexture.getTextureData().textureTarget)) {
...
void ofApp::exit() {
    ...
    // ====== SPOUT =====
	ReleaseReceiver(); // Release the receiver
