...
void testApp::setup(){
	...
	bResized			= false;						// No window resize yet
...
void testApp::draw() {
	...
	if(!bSenderInitialized) {
		...
		// Create a local texture for sending
		InitGLtexture(senderTexture, g_Width, g_Height);
	}
	// Sender initialization will only fail if something is wrong

	// Has the window been resized - update the sender if so
	if(bResized) {
		...
		// Update the sender
		spoutsender->UpdateSender(senderName, g_Width, g_Height);
...
{
	if(w != g_Width || h != g_Height) bResized = true;
}
