...
void testApp::draw() {
	...
	// If the name is NULL it will connect to the active sender. 
	...
	if(bInitialized) {
		...
		if(spoutreceiver->ReceiveTexture(SenderName, width, height, myTexture.getTextureData().textureID, myTexture.getTextureData().textureTarget)) {
			...
			bInitialized = false;
			// Null the sender name if you want to pick up the next active sender
			// Otherwise leave it and the next time that sender starts, it will be detected.
			SenderName[0] = 0; 
			return;
		}
	}
